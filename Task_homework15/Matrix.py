...
class Matrix:
    ...
    def __add__(self, other):
        if len(self._matr) != len(other._matr) or len(self._matr[0]) != len(other._matr[0]):
            logger.error(f'Невозможно выполнить сложение матриц, поскольку размеры матриц несовместимы:  [{len(self._matr)}][{len(self._matr[0])}] !=  [{len(other._matr)}][{len(other._matr[0])}] ')
    ...
    def __mul__(self, other):
        if len(self._matr[0]) != len(other._matr):
            logger.error(f'Невозможно выполнить умножение матриц, поскольку размеры матриц несовместимы: [{len(self._matr)}][{len(self._matr[0])}] !=  [{len(other._matr)}][{len(other._matr[0])}]')
        else:
